...
巡检先回读内存上限和 Jetsam 优先级，达到目标时不重复写入。单侧内存上限设为 0 时，插件保留该侧当前内核值；若无法回读，则跳过整次内存上限写入。`-1` 在 XNU 中表示恢复系统默认上限，并非无限制。Jetsam 的后台提升和冻结设置仍在强制巡检时重申，因为单独的 Jetsam 数值不足以证明这些状态未变化。
